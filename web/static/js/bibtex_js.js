...
/*
 Based on bibtex_js originally by henrik.muehe from http://code.google.com/p/bibtex-js/
 Modified by Simon Walton to include additional functionality (marked with my name)
*/
...
function BibtexParser() {
  ...
  this.entry_body = function() {
	...
	this.entries[this.currentEntry].bibtex_id = this.currentEntry;
    this.match(",");
    this.key_value_list();
  }
  ...
  this.bibtex = function() {
	...
	var start_pos = this.pos;
	  ...
	  // simon: save original BibTeX entry for the popup
	  this.entries[this.currentEntry].entry_body = this.input.substring(start_pos,this.pos);
    }
  }
}
...
function BibtexDisplay() {
  ...
  this.displayBibtex = function(input, output) {
    ...
    for (var entryKey in entries) {
	  ...
	  // simon: add a reference ID badge using Bootstrap
	  var yearTag = tpl.find("span:not(a).ref_begin");
	  $("<a/>", {
		 "href":"#",
		 "class":"badge badge-info bibtex_js_popover",
		 "rel":"popover",
		 "title":"BibTeX",
		 "html":true,
		 "data-content":"<tiny>" + entry.entry_body + "</tiny>",
			text:entryIdx.toString()}
		).appendTo(yearTag).popover();
...
if (typeof jQuery == 'undefined') {  
  ...
  $(document).ready(function () {
	...
	// simon: added ref_begin span for the reference ID badge placeholder
